...
public class VisibleVerticesRaycaster : MonoBehaviour
{
    ...
    void HandleKeyPress()
    {
        if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter))
        {
            ...
            foreach (MeshRenderer meshRenderer in GameObject.FindObjectsOfType<MeshRenderer>())
            {
                ...
                if (mesh != null)
                {
                    ...
                    foreach (var vertexInfo in vertexInfoMap.Values)
                    {
                        ...
                        // If vertex is visible, add it to the visibleVertices list
                        if (vertexInfo.IsVisible)
                        {
                            visibleVertices.Add(vertexInfo.Vertex);
                        }
                    }
                }
            }
            ...
            foreach (var vertexInfo in vertexInfoMap.Values)
            {
                sb.AppendLine($"Order: {vertexInfo.Order}, Position: {vertexInfo.Vertex}, Owner ID: {vertexInfo.Owner.GetInstanceID()}, Distance to Camera: {vertexInfo.DistanceToCamera}, Connected Vertices: {string.Join(", ", vertexInfo.ConnectedVertices)}");
            }
